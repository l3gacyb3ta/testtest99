 ...
 * tickets channel with a "Mark as helped" button. Thread replies and
 * anything from a bot (including our own thread acks) are conversation on
 * an existing ticket, not a new one. */
async function handleHelpMessage(event: SlackEvent) {
  ...
  const blocks: SlackBlock[] = [
    {
      ...
      text: {
        type: "mrkdwn",
        text: `*New ticket* from <@${event.user}> in <#${event.channel}>\n>${event.text ?? ""}`,
      },
    },
    {
      type: "actions",
  ...
  await postMessage(ticketsChannel, `New ticket from <@${event.user}>`, {
    blocks,
  });
}
